...
const MyPrivatePosts=  () => {
	...
	async function getMyPrivatePosts()
	{
		var requestOptions = {
		  mode:'cors',
		  method: 'POST',
		...
		const status=await fetch("http://lokidev.herokuapp.com/private_posts/", requestOptions).then( response => {return response.json();})
		let keys=["private_post_id","views","likes","url"];
		let post_details=[];
		console.log(status);
		if (status==0)
		{
			setInfoMsg("You have not made any private posts");
		}
	...
	return (
		<IonContent>
			<p>My Private Posts</p>
			{postDetails}
			<button class="myButton" onClick={() => getMyPrivatePosts()}>See them</button>
			<br/>
			<b>{infoMsg}</b>
